...
import SignalLines from "../assets/SignalLines.png";
const list = [
  {
    title: "Lorem Ipsum",
...
const Benefits = ({ position }) => {
  return (
    <div
      className={`${position} w-full max-w-[1230px] h-auto mx-auto flex justify-between items-center my-20 px-4 `}
    >
      {/* left side */}
      ...
      <div className="relative w-[550px] h-auto">
        <h2 className="absolute top-[-82px] left-[60px] font-semibold text-[36px]">
          The <span className="text-blue-color">Benefits</span> of Utilizing
          Webapp for clinics
        </h2>
        <div className="w-[600px] h-auto mt-6 relative">
          <img
            src={SignalLines}
            className="absolute top-[-80px] left-[30px] z-10"
          />
          {list.map((item, index) => (
            <div key={index} className="flex gap-3 mb-6 z-30">
              <div className="rounded-full bg-blue-color w-[100px] text-white h-[100px] mt-4 flex justify-center items-center shadow-lg shadow-neutral-300 z-40">
